...
#Synopsis
The aim of this document is to provide a summary of all the analyses used in writing up the results for Experiment 2 in the dissertation document.The analyses are brokwn down into several parts:
...
```{r}
EyeData <- read.csv("Exp2DriftCorrected.csv", na.strings = c("."))[,c('RECORDING_SESSION_LABEL', 'IA_FIRST_RUN_START_TIME', 'IA_FIRST_RUN_START_TIME', 'IA_FIRST_RUN_END_TIME', 'IA_SECOND_RUN_START_TIME', 'IA_SECOND_RUN_END_TIME', 'IA_THIRD_RUN_START_TIME', 'IA_THIRD_RUN_END_TIME', 'IA_LABEL', 'RESPONSE_ACC', 'IA_DWELL_TIME', 'TRIAL_INDEX', 'target', 'RESPONSE_RT', 'trialtype','Bilingual')]
...
SubjectRT
mean(SubjectRT$MeanRT) #average click times were 1374ms. 
```
...
1. Can we observe competition effects even if the spoken target was presented before the onset of the display?

2. Is having bilinguals in this experiment an issue? There were 3 subjects who reported being bilingual, though they spoke English at home. 
...
g1<-ggplot(OnlySubjects, aes(x=time, y=Prob, shape=`Interest Area`, color=`Interest Area`)) +
    ...
    geom_vline(xintercept = 2000+682+200+600)+
    geom_vline(xintercept = 4800)+theme_bw()+
...
#plot from time 1000(fixation cross)+1000(word display with no sound)+682(click on duration)+200(pause duration)+200(time it takes to initiate fixation. This plots the fixations starting from the onset of the word till about 4800ms
OnlySubjects$time2<-OnlySubjects$time-3500
g2<-ggplot(OnlySubjects, aes(x=time2, y=Prob, shape=`Interest Area`, color=`Interest Area`)) +
    geom_point(size=3) +
    geom_errorbar(aes(ymin=LCI, ymax=UCI)) + facet_grid(.~trialtype)+
    scale_x_continuous(breaks=seq(0,1300,100), limits=c(0,1300), name="time from word display onset(ms)")+
    theme_bw()+
    scale_fill_manual(values=cbPalette)+scale_colour_manual(values=cbPalette)+
    scale_y_continuous(name="Fixation Proportion")+
...
ggplot(OnlySubjectsB, aes(x=time, y=Prob, shape=IA_LABEL, color=IA_LABEL)) +
    ...
    scale_x_continuous(limits = c(2000+682+200+600, 4800))+
    theme_bw()
```
...
model<-CleanDataProb %>% 
    filter(trialtype=="Experimental", IA_LABEL=="COMPET_IA " | IA_LABEL=="TARGET_IA ", 
           time>=2000+682+200+600 & time <=4800)
...
#find the summary for model 3
summary(Model3)
#find the RMSE
sqrt(mean((model$Prob-fitted(Model3))^2))
...
Model 3 performs the best, though LMER is not very susceptible to corss-validation, so the addition of more polynomials might be leading to overfitting. More complex models could not be fitted due to convergence issues.
...
levels(ModelLong$Model) <- list(`Model 1 Intercept`="Model1Fitted", `Model 2 Linear`="Model2Fitted", `Model 3 Quadratic`="Model3Fitted", `Model 4 Cubic (not converged)`="Model4Fitted", `Model 5 Cubic (not converged)`="Model5Fitted", `Model 6 Cubic (not converged)`="Model6Fitted", `Random Forest Complex`="RF1",`Random Forest Simple`="RF2")
...
GCAModels$time2<-GCAModels$time-3500
...
RFModels$time2<-RFModels$time-3500
