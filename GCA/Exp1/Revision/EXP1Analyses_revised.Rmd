...
#Synopsis
The aim of this document is to provide a summary of all the analyses used in writing up the results for Experiment 1 in the dissertation document.The analyses are brokwn down into several parts:
...
SubjectRT<- CorrectEyeData %>% group_by(Subject) %>% 
    summarise(MeanRT=mean(RESPONSE_RT)) %>% arrange(MeanRT)
SubjectRT
mean(SubjectRT$MeanRT)
#Rt since participants were presented with the spoken target (take out 682 for click time and 200 for the pause)
mean(SubjectRT$MeanRT)-682-200
```
...
1. What participants do during the period before the onset of the word is presented?

2. Are participants's attention drawn to similar orthographic words before the onset of the target?

3. Is having bilinguals in this experiment an issue? There were 3 subjects who reported being bilingual, though they spoke English at home. 
...
g1<-ggplot(OnlySubjects, aes(x=time, y=Prob, shape=`Interest Area`, color=`Interest Area`)) +
    ...
    geom_vline(xintercept = 2900)+
    geom_vline(xintercept = 4200)+theme_bw()+
...
#now create a plot that gives the fixation proportions from 1.9s (1s fixation corss+682 clikc on+200ms pause)
OnlySubjects$time2<-OnlySubjects$time-2900

g2<-ggplot(OnlySubjects, aes(x=time2, y=Prob, shape=`Interest Area`, color=`Interest Area`)) +
    geom_point(size=3) +
    geom_errorbar(aes(ymin=LCI, ymax=UCI)) + facet_grid(.~trialtype)+
    scale_x_continuous(breaks=seq(0,1300,100), limits=c(0,1300), name="time from spoken target onset(ms)")+
    theme_bw()+
    scale_fill_manual(values=cbPalette)+scale_colour_manual(values=cbPalette)+
    scale_y_continuous(name="Fixation Proportion")+
...
png(filename="plot3Experiment1.png",width=900,height=524)
grid.arrange(g1, g2, nrow=2, ncol=1, top="Fixation proportion for each trial type")
dev.off()
```  

Create a figure showing the fixations during the preview period. 
...
ggplot(OnlySubjectsB, aes(x=time, y=Prob, shape=IA_LABEL, color=IA_LABEL)) +
    ...
    scale_x_continuous(limits = c(1000+1000+682+200+200, 4000))+
    geom_vline(xintercept = 3082)+
    geom_vline(xintercept = 4000)+theme_bw()
```

There doesn't seem to be any substantial differences between bilinguals and natives. There are only 3 bilingual subjects, so the ammount of variability for bilingual is larger, but other than that, there is nothing to suggest that these subjects should be removed. 
...
Exploratory graphs (see above) indicate that there are no effects are observed before the presentation of the speech onset, so proportions are examined from 3100 (about 218 ms after onset of the target) until 4000ms.
...
Model 5. Model 4+addition of the third poly in the fixed and in the random structure(subject only)

Model 6. Model 5+addition of the third poly in the fixed and random structure(subject:condition only)
...
Model 6 performs the best, though LMER is not very susceptible to corss-validation, so the addition of more polynomials might be leading to overfitting
...
Model 5. Model 4+addition of the fourth poly in the fixed and in the random structure. Model fails to converge

Model 6. Model 5+addition of the fourth poly in the subject random effect structure.

Model 7. Model 7+addition of the fourth poly in the subject:condition random effect structure
...
summary(Model7)
#find the RMSE
sqrt(mean((modelData$Prob-fitted(Model7))^2))
...
modelData$Model6Fitted<-fitted(Model6)
modelData$Model7Fitted<-fitted(Model7)
```

Model 7 seems to perform the best, while also converging to a result. Next, I will examine what happens in random forests.
...
The fourth poly seems to be redundant, and the same could be said about X3. I rerun the random forest without these two variables. 
...
The cross-validated random forest model fewer components performs a bit better than the one that has more components. This seems to suggest that Model 3 in LMER would be the most appropriate to use if there are concerns about over-fitting.
...
levels(ModelLong$Model) <- list(`Model 1 Intercept`="Model1Fitted", `Model 2 Linear`="Model2Fitted", `Model 3 Quadratic`="Model3Fitted", `Model 4 Cubic (not converged)`="Model4Fitted", `Model 5 Cubic`="Model5Fitted", `Model 6 Cubic`="Model6Fitted", `Random Forest Complex`="RF1",`Random Forest Simple`="RF2")
...
The code chunk below will plot and save the graph for the GCA models

```{r, warning=FALSE, cache=TRUE}
#color palette for color-blind
cbPalette <- c("#999999", "#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00", "#CC79A7")
#create plot1 for Experiment 1, GCA
...
GCAModels$time2<-GCAModels$time-3100
...
ggplot(GCAModels, aes(x=time2, y=`Fixation Probability`, shape=`Interest Area`, color=`Interest Area`)) +
    ...
    theme_bw()+
    scale_x_continuous(name="time 200ms after spoken target onset(ms)", breaks=seq(0,1000,100), limits=c(0,900))+
    theme(legend.text = element_text(size = 12))+
    theme(legend.title = element_text(size=12, face="bold"))+
...
RFModels$time2<-RFModels$time-3100
...
```{r, cache=TRUE}
ModelLong<- modelData %>% gather(Model,Predictions,11:19)
#rename the models so that they are more clear
levels(ModelLong$Model) <- list(`Model 1 Intercept`="Model1Fitted", `Model 2 Linear`="Model2Fitted", `Model 3 Quadratic`="Model3Fitted", `Model 4 Cubic`="Model4Fitted", `Model 5 Quartic (not converged)`="Model5Fitted", `Model 6 Quartic`="Model6Fitted",`Model 7 Quartic`="Model7Fitted",`Random Forest Complex`="RF1",`Random Forest Simple`="RF2")
...
The code chunk below will plot and save the graph for the models
