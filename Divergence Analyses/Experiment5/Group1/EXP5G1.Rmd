...
```{r}
CleanData<- CleanData %>% gather(time,value,8:248) %>% filter(trialtype=="Experimental") %>% select(-c(trialtype))
```
...
CleanDataSubset<-CleanData %>% filter(time>=4100 & time <=5000)

#change the tiem variable so that it starts at 0.
CleanDataSubset$time<-CleanDataSubset$time-4100
...
testSubset2<-testSubset %>% filter(!c(value==0 & TrackLoss==1)) %>%
    ...
    filter(!c(IA_LABEL=="UNREL3_IA " & TrackLoss==0)) %>%
    filter(!c(IA_LABEL=="UNREL4_IA " & TrackLoss==0)) 
#now we have a trackloss column; in the following step, I will create the AOI Columns
#first, create filled out empty colums
namevector<-c("Target","Competitor","Distractor1","Distractor2","Distractor3","Distractor4")
...
testSubset3<- testSubset2 %>% mutate(
    ...
    Distractor=ifelse(c(IA_LABEL=="UNREL3_IA " & TrackLoss==1),TRUE,FALSE),
    Distractor=ifelse(c(IA_LABEL=="UNREL4_IA " & TrackLoss==1),TRUE,FALSE),
...
clust_analysis <- analyze_time_clusters(df_timeclust, samples=10000, within_subj=TRUE, paired=TRUE, quiet=TRUE) #

distribution<-plot(clust_analysis) + theme_light()

png(filename="CompvsDistractorExp5g1.png",width=900,height=524)
