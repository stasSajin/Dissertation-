...
```{r}
CleanData<- CleanData %>% gather(time,value,11:331) %>% filter(trialtype=="Experimental") %>% select(-c(trialtype))
```
...
CleanDataSubset<-CleanData %>% filter(time>=4200 & time <=5500)

#change the tiem variable so that it starts at 0.
CleanDataSubset$time<-CleanDataSubset$time-4200
